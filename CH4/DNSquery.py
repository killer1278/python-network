...
def getreverse(query):
    """Give the query, returns an appropriate reverse lookup string
...
while len(queries):
    ...
    if rev:
        print("IP address give; doing reverse lookup using", rev)
        query = rev
    ...
    for answer in answers:
        ...
        if answer['typename'] == 'MX':
            print(formatline(count, asnwer['typename'],'Mail server',
                '%s, priority %d' %(answer['data'][1], answer['data'][0])))
        elif answer['typename'] in descriptions:
            print(formatline(count, asnwer['typename'],
