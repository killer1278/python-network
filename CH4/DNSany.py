...
def hierquery(qstring, qtype):
    ...
    If no answers are found, removes the most specific component(the part before the leftmost period) 
    and restries the query with the result. If the topmost query fails, returns None."""
    reqobj = DNS.Request()
    try:
...
def findnameservers(hostname):
    """Attempts to determine the authoritative nameservers for a given hostname. Returns    None on failure."""
    return hierquery(hostname, DNS.Type.NS)


def getrecordsfromnameserver(qstring, qtype, nslist):
    """Given a list of nameservers in nslist, executes the query requested by qstring and qtype on each in order,
    returning the data from the first server that returned 1 or more answers. If no server returned any answers, returns []."""
    for ns in nslist:
        reqobj = DNS.Request(server = nslist)
        try:
